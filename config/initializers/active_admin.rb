ActiveAdmin.setup do |config|
  ...
  config.namespace :admin do |admin|
    ...
    admin.build_menu do |menu|
      menu.add id: :language_switcher,
               label: proc{ I18n.locale == :en ? "日本語" : "English" },
               url: proc{ 
                 new_locale = I18n.locale == :en ? :ja : :en
                 current_path = Rails.application.routes.recognize_path(request.url)
                 url_for(current_path.merge(locale: new_locale))
               },
               priority: 100
    end
  end
